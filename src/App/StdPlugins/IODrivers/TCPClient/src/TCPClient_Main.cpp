...
extern "C"
{
            ...
            unsigned int Version)
    {
        ...
        if(g_TCPC_System->GetExperimentalID()>0 &&
                g_TCPC_System->GetExperimentalID()<1)
        {
            return 0xFFFFFFFF;
        }
            ...
            t_PIKVList *Options,char *URI,unsigned int MaxURILen)
{
    ...
    if(strlen(TCPCLIENT_URI_PREFIX)+1+2+strlen(AddressStr)+1+strlen(PortStr)+1>=
            MaxURILen)
    {
        return false;
    }
