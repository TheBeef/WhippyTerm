        ...
        t_IOSystemHandle *IOHandle)
{
    ...
    try
    {
        ...
        NewData->ThreadHandle=CreateThread(NULL,0,UDPServer_OS_PollThread,
...
PG_BOOL UDPServer_Open(t_DriverIOHandleType *DriverIO,const t_PIKVList *Options)
{
    ...
    if((OurData->DataSockFD=socket(AF_INET,SOCK_DGRAM,0))==INVALID_SOCKET)
        return false;

    if(MulticastBool)
    {
        /* Multicast must be ReuseAddressBool so we force it */
    ...
    if(MulticastBool)
    {
        ...
        if(setsockopt(OurData->DataSockFD,IPPROTO_IP,IP_ADD_MEMBERSHIP,
...
int UDPServer_Read(t_DriverIOHandleType *DriverIO,uint8_t *Data,int MaxBytes)
{
    ...
    if(OurData->BytesInBuffer==0)
    {
        ...
        if(select(OurData->DataSockFD+1,&fds,NULL,NULL,&tv)!=0)
        {
            ...
            {
                Byte2Ret=recv(OurData->DataSockFD,(char *)OurData->ReadBuffer,
                        sizeof(OurData->ReadBuffer),0);
