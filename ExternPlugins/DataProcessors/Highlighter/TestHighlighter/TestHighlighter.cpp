...
/*** DEFINES                  ***/
#define REGISTER_PLUGIN_FUNCTION_PRIV_NAME      TestHighlighter             // The name to append on the RegisterPlugin() function for built in version
...
/*** FUNCTION PROTOTYPES      ***/
const struct DataProcessorInfo *TestHighlighter_GetProcessorInfo(unsigned int *SizeOfInfo);
void TestHighlighter_ProcessByte(t_DataProcessorHandleType *DataHandle,
        const uint8_t RawByte,uint8_t *ProcessedChar,int *CharLen,
        PG_BOOL *Consumed);
PG_BOOL TestHighlighter_ProcessKeyPress(t_DataProcessorHandleType *DataHandle,
        const uint8_t *KeyChar,int KeyCharLen,e_UIKeys ExtendedKey,
        uint8_t Mod);
t_DataProSettingsWidgetsType *TestHighlighter_AllocSettingsWidgets(t_WidgetSysHandle *WidgetHandle,t_PIKVList *Settings);
void TestHighlighter_FreeSettingsWidgets(t_DataProSettingsWidgetsType *PrivData);
void TestHighlighter_StoreSettings(t_DataProSettingsWidgetsType *PrivData,t_PIKVList *Settings);

/*** VARIABLE DEFINITIONS     ***/
struct DataProcessorAPI m_TestHighlighterCBs=
{
    /* V1 */
    NULL,
    NULL,
    TestHighlighter_GetProcessorInfo,
    TestHighlighter_ProcessKeyPress,
    TestHighlighter_ProcessByte,
    NULL, // ProcessIncomingBinaryByte
    /* V2 */
    NULL,                                       // ProcessOutGoingData
    TestHighlighter_AllocSettingsWidgets,
    TestHighlighter_FreeSettingsWidgets,
    TestHighlighter_StoreSettings,
};
...
/*******************************************************************************
 * NAME:
 *    TestHighlighter_RegisterPlugin
 *
 * SYNOPSIS:
 *    unsigned int TestHighlighter_RegisterPlugin(const struct PI_SystemAPI *SysAPI,
...
extern "C"
{
            ...
            unsigned int Version)
    {
        ...
        m_DPS->RegisterDataProcessor("TestHighlighter",
                &m_TestHighlighterCBs,sizeof(m_TestHighlighterCBs));
 ...
 *
 * SYNOPSIS:
 *    const struct DataProcessorInfo *TestHighlighter_GetProcessorInfo(
 ...
 *
 *  SYNOPSIS:
 *    void TestHighlighter_ProcessByte(
 ...
 *
 * SYNOPSIS:
 *   PG_BOOL TestHighlighter_ProcessKeyPress(
...
struct TestHighlighterWidgetData
{
...
{
    struct TestHighlighterWidgetData *WData=(struct TestHighlighterWidgetData *)UserData;
...
t_DataProSettingsWidgetsType *TestHighlighter_AllocSettingsWidgets(t_WidgetSysHandle *WidgetHandle,t_PIKVList *Settings)
{
    struct TestHighlighterWidgetData *WData;
    const char *C1;
    const char *C2;

    m_DPS->SetCurrentSettingsTabName("FIVE!");

    WData=new TestHighlighterWidgetData;
    WData->FirstTabWidgetHandle=WidgetHandle;

    WData->TestCheckbox=m_TestChars_UIAPI->AddCheckbox(WidgetHandle,"Test Checkbox 5",TheEventCB,(void *)WData);

    WData->TabWidgetHandle2=m_DPS->AddNewSettingsTab("SIX!");
    WData->TestCheckbox2=m_TestChars_UIAPI->AddCheckbox(WData->TabWidgetHandle2,"Test Checkbox 6",NULL,NULL);

    C1=m_TestChars_SysAPI->KVGetItem(Settings,"Checkbox5");
    C2=m_TestChars_SysAPI->KVGetItem(Settings,"Checkbox6");
...
void TestHighlighter_FreeSettingsWidgets(t_DataProSettingsWidgetsType *PrivData)
{
    struct TestHighlighterWidgetData *WData=(struct TestHighlighterWidgetData *)PrivData;
...
void TestHighlighter_StoreSettings(t_DataProSettingsWidgetsType *PrivData,t_PIKVList *Settings)
{
    ...
    sprintf(buff,"%d",C1);
    m_TestChars_SysAPI->KVAddItem(Settings,"Checkbox5",buff);

    sprintf(buff,"%d",C2);
    m_TestChars_SysAPI->KVAddItem(Settings,"Checkbox6",buff);
}
