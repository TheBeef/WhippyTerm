...
/*** DEFINES                  ***/
#define REGISTER_PLUGIN_FUNCTION_PRIV_NAME      TestCharsDecoder // The name to append on the RegisterPlugin() function for built in version
...
/*** FUNCTION PROTOTYPES      ***/
const struct DataProcessorInfo *TestCharsEncoder_GetProcessorInfo(unsigned int *SizeOfInfo);
void TestCharsEncoder_ProcessByte(t_DataProcessorHandleType *DataHandle,
        const uint8_t RawByte,uint8_t *ProcessedChar,int *CharLen,
        PG_BOOL *Consumed);
PG_BOOL TestCharsEncoder_ProcessKeyPress(t_DataProcessorHandleType *DataHandle,
        const uint8_t *KeyChar,int KeyCharLen,e_UIKeys ExtendedKey,
        uint8_t Mod);
t_DataProSettingsWidgetsType *TestCharsEncoder_AllocSettingsWidgets(t_WidgetSysHandle *WidgetHandle,t_PIKVList *Settings);
void TestCharsEncoder_FreeSettingsWidgets(t_DataProSettingsWidgetsType *PrivData);
void TestCharsEncoder_StoreSettings(t_DataProSettingsWidgetsType *PrivData,t_PIKVList *Settings);

/*** VARIABLE DEFINITIONS     ***/
struct DataProcessorAPI m_TestCharsDecoderCBs=
{
    /* V1 */
    NULL,
    NULL,
    TestCharsEncoder_GetProcessorInfo,
    TestCharsEncoder_ProcessKeyPress,
    TestCharsEncoder_ProcessByte,
    NULL, // ProcessIncomingBinaryByte
    /* V2 */
    NULL,                                       // ProcessOutGoingData
    TestCharsEncoder_AllocSettingsWidgets,
    TestCharsEncoder_FreeSettingsWidgets,
    TestCharsEncoder_StoreSettings,
};

struct DataProcessorInfo m_TestCharsEncoder_Info=
{
    "Test Encoder",
    "A test encoder for testing WhippyTerm encoders",
    "Used for testing encoders in WhippyTerm",
...
/*******************************************************************************
 * NAME:
 *    TestCharsEncoder_RegisterPlugin
 *
 * SYNOPSIS:
 *    unsigned int TestCharsEncoder_RegisterPlugin(const struct PI_SystemAPI *SysAPI,
...
extern "C"
{
            ...
            unsigned int Version)
    {
        ...
        m_DPS->RegisterDataProcessor("TestCharsDecoder",
                &m_TestCharsDecoderCBs,sizeof(m_TestCharsDecoderCBs));
 ...
 *
 * SYNOPSIS:
 *    const struct DataProcessorInfo *TestCharsEncoder_GetProcessorInfo(
        ...
        unsigned int *SizeOfInfo)
{
    *SizeOfInfo=sizeof(struct DataProcessorInfo);
    return &m_TestCharsEncoder_Info;
}
 ...
 *
 *  SYNOPSIS:
 *    void TestCharsEncoder_ProcessByte(
 ...
 *
 * SYNOPSIS:
 *   PG_BOOL TestCharsEncoder_ProcessKeyPress(
...
struct TestcharsEncoderWidgetData
{
...
{
    struct TestcharsEncoderWidgetData *WData=(struct TestcharsEncoderWidgetData *)UserData;
...
t_DataProSettingsWidgetsType *TestCharsEncoder_AllocSettingsWidgets(t_WidgetSysHandle *WidgetHandle,t_PIKVList *Settings)
{
    struct TestcharsEncoderWidgetData *WData;
    const char *C1;
    const char *C2;

    m_DPS->SetCurrentSettingsTabName("ONE!");

    WData=new TestcharsEncoderWidgetData;
    WData->FirstTabWidgetHandle=WidgetHandle;

    WData->TestCheckbox=m_TestChars_UIAPI->AddCheckbox(WidgetHandle,"Test Checkbox 1",TheEventCB,(void *)WData);

    WData->TabWidgetHandle2=m_DPS->AddNewSettingsTab("TWO!");
    WData->TestCheckbox2=m_TestChars_UIAPI->AddCheckbox(WData->TabWidgetHandle2,"Test Checkbox 2",NULL,NULL);

    C1=m_TestChars_SysAPI->KVGetItem(Settings,"Checkbox1");
    C2=m_TestChars_SysAPI->KVGetItem(Settings,"Checkbox2");
...
void TestCharsEncoder_FreeSettingsWidgets(t_DataProSettingsWidgetsType *PrivData)
{
    struct TestcharsEncoderWidgetData *WData=(struct TestcharsEncoderWidgetData *)PrivData;
...
void TestCharsEncoder_StoreSettings(t_DataProSettingsWidgetsType *PrivData,t_PIKVList *Settings)
{
    ...
    sprintf(buff,"%d",C1);
    m_TestChars_SysAPI->KVAddItem(Settings,"Checkbox1",buff);

    sprintf(buff,"%d",C2);
    m_TestChars_SysAPI->KVAddItem(Settings,"Checkbox2",buff);
}
