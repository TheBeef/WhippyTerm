...
/*** DEFINES                  ***/
#define REGISTER_PLUGIN_FUNCTION_PRIV_NAME      TestOther             // The name to append on the RegisterPlugin() function for built in version
...
/*** FUNCTION PROTOTYPES      ***/
const struct DataProcessorInfo *TestOther_GetProcessorInfo(unsigned int *SizeOfInfo);
void TestOther_ProcessByte(t_DataProcessorHandleType *DataHandle,
        const uint8_t RawByte,uint8_t *ProcessedChar,int *CharLen,
        PG_BOOL *Consumed);
PG_BOOL TestOther_ProcessKeyPress(t_DataProcessorHandleType *DataHandle,
        const uint8_t *KeyChar,int KeyCharLen,e_UIKeys ExtendedKey,
        uint8_t Mod);
t_DataProSettingsWidgetsType *TestOther_AllocSettingsWidgets(t_WidgetSysHandle *WidgetHandle,t_PIKVList *Settings);
void TestOther_FreeSettingsWidgets(t_DataProSettingsWidgetsType *PrivData);
void TestOther_StoreSettings(t_DataProSettingsWidgetsType *PrivData,t_PIKVList *Settings);

/*** VARIABLE DEFINITIONS     ***/
struct DataProcessorAPI m_TestOtherCBs=
{
    /* V1 */
    NULL,
    NULL,
    TestOther_GetProcessorInfo,
    TestOther_ProcessKeyPress,
    TestOther_ProcessByte,
    NULL, // ProcessIncomingBinaryByte
    /* V2 */
    NULL,                                       // ProcessOutGoingData
    TestOther_AllocSettingsWidgets,
    TestOther_FreeSettingsWidgets,
    TestOther_StoreSettings,
};

struct DataProcessorInfo m_TestOther_Info=
{
    "Test Other",
    "A test other for testing WhippyTerm emulations",
    "Used for testing other in WhippyTerm",
...
/*******************************************************************************
 * NAME:
 *    TestOther_RegisterPlugin
 *
 * SYNOPSIS:
 *    unsigned int TestOther_RegisterPlugin(const struct PI_SystemAPI *SysAPI,
...
extern "C"
{
            ...
            unsigned int Version)
    {
        ...
        m_DPS->RegisterDataProcessor("TestOther",
                &m_TestOtherCBs,sizeof(m_TestOtherCBs));
 ...
 *
 * SYNOPSIS:
 *    const struct DataProcessorInfo *TestOther_GetProcessorInfo(
 ...
 *
 * SYNOPSIS:
 *   PG_BOOL TestOther_ProcessKeyPress(
...
{
    struct TestOtherWidgetData *WData=(struct TestOtherWidgetData *)UserData;
...
t_DataProSettingsWidgetsType *TestOther_AllocSettingsWidgets(t_WidgetSysHandle *WidgetHandle,t_PIKVList *Settings)
{
    struct TestOtherWidgetData *WData;
    const char *C1;
    const char *C2;

    m_DPS->SetCurrentSettingsTabName("SEVEN!");

    WData=new TestOtherWidgetData;
    WData->FirstTabWidgetHandle=WidgetHandle;

    WData->TestCheckbox=m_TestChars_UIAPI->AddCheckbox(WidgetHandle,"Test Checkbox 7",TheEventCB,(void *)WData);

    WData->TabWidgetHandle2=m_DPS->AddNewSettingsTab("EIGHT!");
    WData->TestCheckbox2=m_TestChars_UIAPI->AddCheckbox(WData->TabWidgetHandle2,"Test Checkbox 8",NULL,NULL);

    C1=m_TestChars_SysAPI->KVGetItem(Settings,"Checkbox7");
    C2=m_TestChars_SysAPI->KVGetItem(Settings,"Checkbox8");
...
void TestOther_FreeSettingsWidgets(t_DataProSettingsWidgetsType *PrivData)
{
    struct TestOtherWidgetData *WData=(struct TestOtherWidgetData *)PrivData;
...
void TestOther_StoreSettings(t_DataProSettingsWidgetsType *PrivData,t_PIKVList *Settings)
{
    ...
    sprintf(buff,"%d",C1);
    m_TestChars_SysAPI->KVAddItem(Settings,"Checkbox7",buff);

    sprintf(buff,"%d",C2);
    m_TestChars_SysAPI->KVAddItem(Settings,"Checkbox8",buff);
}
